...
# Problem 1
Do exercise 12 in Chapter 5 in the textbook using the
following market basket transactions and a minimum support threshold of 35%. You must show
...
## a. 
Using the transactions in Problem 1, draw a contingency table for each of the followingrules: {b} → {e}, {a} → {d}, {e} → {d}.
...
## b.
Using the contingency tables and the three association rules in Question (a) of Problem 2,
for each of the measures (Support, Confidence, Interest, and IS), compute the measure
values for those rules and rank the rules based on their measure values with rank 1 being the best and 3 the worst. Show your work manually in detail. For each measure, show a
table that has the following format, where <Measure> is one of the above four measures:
...
## c. Given the rankings obtained in Question (b) of Problem 2, for each measure (Support,
Interest, and IS), compute the correlation between the ranking of Confidence and the
rankings of that measure. Which of those three measures is the most correlated with
Confidence? Which measure is the least correlated with Confidence? Justify your answers.


# Problem 3
## b. Using a programming language of your choice (e.g. C, C++, Java, Python, or R), write a
program to perform the following tasks (i-iii) concerning the generation of frequent
itemsets and association rules when applied to help a business recommend products to its
customers:

### i. Implement a modified version of the Apriori Algorithm described on page 368 of
the textbook for frequent itemset generation so that the algorithm will generate only
frequent itemsets of size less than or equal to N. Implement this version as a
function named ModifiedApriori.
```{r problem-3-bII, echo = FALSE, eval=TRUE, results = 'asis'}
```